...
lbry.wallet_list = () => { return _request("wallet_list") }
...
// Returns:
//     (str) New wallet address in base58
lbry.wallet_balance = (optional_paramaters) => {
  if (optional_paramaters)
    return _request("wallet_balance", optional_paramaters)
...
lbry.file_set_status = (status, optional_paramaters) => {
  const params = {
    status: status
  }
  Object.assign(params, optional_paramaters);
  return _request("file_set_status", params)
}
...
//    'claim_address' (optional): (str) address where the claim is sent to, if not specified new address wil automatically be created
//
//Returns:
//    (dict) Dictionary containing result of the claim
//    {
...
lbry.wallet_unused_address = () => { return _request("wallet_unused_address") }
